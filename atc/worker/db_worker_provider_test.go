...
var _ = Describe("DBProvider", func() {
	...
	Describe("FindWorkersForResourceCache", func() {
		...
		JustBeforeEach(func() {
			foundWorkers, findErr = provider.FindWorkersForResourceCache(logger, fakeDBTeam.ID(), 1)
		})
		...
		Context("when there there is no underlying error", func() {
			...
			It("should find both workers", func() {
				...
				Expect(workerNames).To(ConsistOf([]string{"some-worker", "some-other-worker"}))
			})

			Context("when one of the worker version is outdated", func() {
				BeforeEach(func() {
					workerVersionOld := "1.0.0"
					fakeWorker2.VersionReturns(&workerVersionOld)
				})
